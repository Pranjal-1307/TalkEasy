...
class CallScreen extends StatefulWidget {
  final String channelId; // Unique call room (User ID)
  final bool isCaller;
...
class _CallScreenState extends State<CallScreen> {
  ...
  Future<void> _initializeAgora() async {
    _engine = createAgoraRtcEngine(); // ✅ Correct initialization
  ...
  token: "007eJxTYLhctdPJ8WqsZrBkZckvq8McN2ZcnHwgZtGKZ5ctrO4wrlFQYDCzNDAwNzFMSzRIMzAxNTROTLNMSk1LSjEzNTUyMrEwdlh/JL0hkJGh6+JTVkYGRgYWIAbxmcAkM5hkAZMcDCGJOdmuicWVDAwAb1YklQ==", // Use an empty string instead of null
    ...
    return Scaffold(
      ...
      body: Center(
        child: _remoteUid == null
            ...
            : Text("User $_remoteUid is in the call"),
      ),
    );
  }
}*/
...
import 'package:talk_easy/services/webrtc_service.dart';
...
class _CallScreenState extends State<CallScreen> {
  ...
  Future<void> _initializeCall() async {
    _webRTCService = WebRTCService(
      ...
      onAddRemoteStream: (stream) {
        setState(() {
          ...
          print("✅ Remote stream received, updating UI!");
        });
      },
      onCallEnded: () {
        if (mounted) {
    ...
    await _webRTCService.init(isAudioOnly: true);
    ...
    setState(() {
      _localStream = _webRTCService.getLocalStream();
    });
  }
  ...
  void _toggleSpeaker() {
    ...
    _webRTCService.setSpeakerphoneOn(isSpeakerOn);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Voice Call")),
      body: Center(
        child: Text(_remoteStream != null ? "User Connected" : "Waiting for user..."),
      ),
      bottomNavigationBar: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          IconButton(icon: Icon(isMicMuted ? Icons.mic_off : Icons.mic), onPressed: _toggleMute),
          IconButton(icon: Icon(isSpeakerOn ? Icons.volume_up : Icons.volume_down), onPressed: _toggleSpeaker),
